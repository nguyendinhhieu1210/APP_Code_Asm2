...
public class BudgetDatabaseHelper extends SQLiteOpenHelper {
    ...
    public boolean updateBudget(String oldGroupName, String newGroupName, String amount, String startDate, String endDate) {
        ...
        if (rows > 0) {
            updateExpensesBudgetName(oldGroupName, newGroupName);
        }
    ...
    private void updateExpensesBudgetName(String oldGroupName, String newGroupName) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("budget_name", newGroupName);
        db.update("expenses", values, "budget_name=?", new String[]{oldGroupName});
        db.close();
    }
